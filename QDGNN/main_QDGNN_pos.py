...
class GraphDataset(Dataset):
    def __init__(self, phase='train', feat=None,method=None, args=None, lists=None, G=None, number=100):
        ...
        if phase == 'train':
            ...
            cur_in = np.zeros((number, feature.shape[0]), dtype=np.int32)
            cur_out = np.zeros((number, feature.shape[0]), dtype=np.int32)
            attr = np.zeros((cur_out.shape[0], feature.shape[0]), dtype=bool)
            i = 0
            for qlists, pos, comm in lists:
                if i>=number:
                    break
                neg = [x for x in range(feature.shape[0]) if x not in comm]
                random.shuffle(neg)
                neg = neg[:args.neg_size]
                for q in qlists:
                    cur_in[i][q] = 1
                for u in pos:
                    cur_out[i][u] = 1
...
def loadQuerys(dataset, root, train_n, val_n, test_n, train_path, test_path, val_path):
    ...
    for line in open(path_train, encoding='utf-8'):
        ...
        if len(train_lists) >= train_n:# + val_n + test_n:
            break
        train_lists.append((qlists, pos_, comm_))
    ...
    for line in open(path_val, encoding='utf-8'):
        ...
        if len(val_lists)>=val_n:
            break
        val_lists.append((qlists, pos_,comm_))
    #'''
...
def main(args):
    ...
    # # #
    # testloader = DataLoader(GraphDataset_(phase='test', ego=ego, method=method),
    #                         batch_size=1, shuffle=False, sampler=None)
    ...
    for epoch in range(args.epoch):
        ...
        for i_iter, batch in enumerate(trainloader):
            ...
            output = output.view(feat.shape[0], args.pos_size+args.neg_size, 1)
            label = label.view(feat.shape[0], args.pos_size+args.neg_size, 1)#'''
    ...
    now = datetime.datetime.now()
    eval_and_test(model, model_dir, evalloader, testloader, best_val_score, test_score, device, nodes_adj, n_nodes)
...
def _test_(model, testloader, best_thr, nodes_adj, n_nodes, device):
    ...
    for i_iter, batch in enumerate(testloader):
        ...
        comm_find = find_community(output, q, n_nodes, nodes_adj, best_thr)
        comm_find = set(comm_find)
        comm_find = list(comm_find)
        comm = set(comm)
        comm = list(comm)
...
def find_community(output_all, q, n_nodes, nodes_adj, best_thr):
    commf = []
    commf.append(q)
    '''
...
def eval_and_test(model, model_dir, evalloader, testloader, best_val_score, test_score, device,
                  nodes_adj, n_nodes):
    # model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='facebook_686')
    parser.add_argument('--root', type=str, default='./data/')
    parser.add_argument('--feats_path', type=str, default='/facebook_686_feats.txt')
    parser.add_argument('--train_path', type=str, default='_3_train_pos.txt')
    parser.add_argument('--test_path', type=str, default='_test.txt')
    parser.add_argument('--eval_path', type=str, default='_3_val_pos.txt')
