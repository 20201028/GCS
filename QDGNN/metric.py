...
def calc_f1_score_iouthr_(output, label, iou_thr=0.5):

    precision_, recall_, f1_score_ = 0.0,0.0,0.0
    for i in range(len(output)):
        output_ = np.array(output[i], dtype=float)
        label_ = np.array(label[i], dtype=float)
        tp = ((output_ > iou_thr) * (label_ > iou_thr)).sum()
        fp = ((output_ > iou_thr) * (label_ < iou_thr)).sum()
        fn = ((output_ < iou_thr) * (label_ > iou_thr)).sum()
        if tp + fp > 0:
            precision = tp * 1. / (tp + fp)
        else:
            precision = 0
        ...
        if precision + recall > 0:
            ...
            f1_score = 0
        precision_=precision_+precision
        recall_=recall_+recall
        f1_score_=f1_score_+f1_score


    return precision_/len(output), recall_/len(output), f1_score_/len(output)
