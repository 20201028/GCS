...
class GraphDataset(Dataset):
    def __init__(self, phase='train', feat=None,method=None, args=None, lists=None, G=None, number=50):
        ...
        if phase == 'train':
            ...
            attr = np.zeros((number, feature.shape[1]), dtype=np.int32)
            i = 0
            #print(lists[0])
            for qlist, attrlists, comm in lists:
                if i>=number:
                    break
                for q in qlist:
                    cur_in[i][q] = 1
...
def loadQuerys(dataset, root, train_n, val_n, test_n, train_path, test_path, val_path):
    ...
    for line in open(path_train, encoding='utf-8'):
        ...
        attrlists = attrlists.split(" ")
        attrlists = [int(x) for x in attrlists]
        comm = comm.split(" ")
        comm = [int(x) for x in comm]
        if len(train_lists) >= train_n:
            break
        qlists = qlists[:1]
        train_lists.append((qlists, attrlists, comm))
    ...
    for line in open(path_val, encoding='utf-8'):
        ...
        if len(val_lists) >= val_n:
            break
        qlists = qlists[:1]
        val_lists.append((qlists, attrlists, comm))
...
def load_data(dataset, root, train_n, val_n, test_n, feats_path, train_path,
    ...
    for line in open(path_feat, encoding='utf-8'):
        if count == 1:
            ...
            feats_node[id] = feats
            for f in emb:
                if f not in nodes_adj_bi:
                    nodes_adj_bi[f] = []
                nodes_adj_bi[f].append(id)
    ...
    print(nodes_feats.shape)

    return nodes_feats, train, val, test, node_in_dim, n_nodes, graphx, nodes_adj, nodes_adj_bi, feats_node_
...
def main(args):
    ...
    # nodes_feats, train, val, test, node_in_dim, n_nodes, g, nodes_adj, nodes_adj_bi, feats_node \
    ...
    # loader2 = CommunityQueryLoader(args.root, args.dataset)
    # nodes_feats, train, val, test, node_in_dim, n_nodes, g, nodes_adj, nodes_adj_bi, feats_node  = loader2.generate_and_load_queries(
    #     args.train_size, args.eval_size, args.test_size
    # )#4:3:3比例划分
# 'graph': {
#     'adj' : graph['adj'],
...
# }
    query_batchs = load_query_batches(args.root, args.dataset)
    query_inf = query_batchs[0]
    graph = query_inf[0]['graph']
    nodes_feats = graph['attributes']
    n_nodes = graph['num_nodes']
    g = nx.Graph()
    edges = graph['edge_index'].t().tolist()  # 转换为 [(0,1), (1,2), ...]
    g.add_edges_from(edges)
    nodes_adj = {}
    for id1, id2 in g.edges:
        if id1 not in nodes_adj:
    ...
    feats_node = {}
    feature_vector = {}
    for node_id, feat_array in enumerate(nodes_feats):
        feature_vector[node_id] = feat_array.tolist()
        feature_indices = [idx for idx, val in enumerate(feature_vector) if val == 1]
        feats_node[node_id] = feature_indices
        for feat_idx in feature_indices:
            nodes_adj_bi.setdefault(feat_idx, []).append(node_id)
    query_list = []
    for q_i in query_inf:
        query = q_i['query']
        community = q_i['target']['community_indices']
        query_list.append((query['nodes'],query['attrs'],community))
    
    train, val, test =query_list[:int(len(query_list)*0.6)], query_list[int(len(query_list)*0.6):int(len(query_list)*0.8)], query_list[int(len(query_list)*0.8):]
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch % 10 == 9):
            ...
            eval_and_test(model, model_dir, evalloader, testloader, best_val_score, test_score, device
                  , nodes_adj, n_nodes, nodes_adj_bi, feats_node)
    ...
    output = args.project_name + '/result/' + args.dataset + args.result
...
def _test_(model, testloader, best_thr, nodes_adj, n_nodes, device, nodes_adj_bi, feats_node):
    ### eval ###
    model.eval()
    all_output = []
    all_label = []
    ...
    for i_iter, batch in enumerate(testloader):
        ...
        comm_find = find_community(output, qlists, n_nodes, nodes_adj, best_thr, nodes_adj_bi, feats_node)
        comm_find = set(comm_find)
        comm_find = list(comm_find)
        comm = set(comm)
        comm = list(comm)
        ...
        Rec = Rec + rec
        print("count", count)
        print("f1, pre, rec", qlists, f1, pre, rec, len(comm), len(comm_find))
...
def find_community(output_all, qlists, n_nodes, nodes_adj, best_thr, nodes_adj_bi, feats_node):
    '''
    commf = []
...
def eval_and_test(model, model_dir, evalloader, testloader, best_val_score, test_score,
                  device, nodes_adj, n_nodes, nodes_adj_bi, feats_node):
    # model.eval()
    ...
    if f1_score > best_val_score[2]:
        ...
        precision, recall, f1_score,nmi_score, ari_score, jaccard_score = \
            _test_(model, testloader, best_thr, nodes_adj, n_nodes, device, nodes_adj_bi, feats_node)
        test2 = time.time()
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--data_set", type=str, default="cora")
    ...
    parser.add_argument('--feats_path', type=str, default='cora_feat.txt')
    ...
    # print("average " + str(args)  + " train_model_running_time " + \
